...
If your environment supports parallel subagents, spawn 3+ in parallel.
Otherwise, run them sequentially while keeping each prompt distinct. Each one
...
in the brief so each subagent names things consistently with the architecture
